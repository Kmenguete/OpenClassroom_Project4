...
class RoundSerializer:
    ...
    serialized players object. As we can see, each object depend on each other to be stored properly in the database.
    """
    ...
    def deserialize(self, serialized_round: dict):
        ...
        date_time_obj = datetime.strptime(
            serialized_round["start_date"], Config.DATE_STRING_FORMAT
        )

        return Tour(
            round_id=serialized_round["round_id"],
            round_name=serialized_round["round_name"],
            start_date=date_time_obj,
            matches=deserialized_matches,
        )
