...
class TournamentService:
    ...
    def create_first_round(self, match_list, round_id):
        """The create_first_round method creates the first round once the user created every player required to
    ...
    def create_next_round(self, round_number, match_list, round_id):
        ...
        next_round = Tour(
            round_id, "Round {}".format(round_number), datetime.now(), match_list
        )
    ...
    def check_if_match_already_happened(self, player_a, player_b):
        ...
        for tour in self.tournament.rounds:
            for match in tour.matches:
                if (match.player_a == player_a and match.player_b == player_b) or (
                    match.player_a == player_b and match.player_b == player_a
                ):
                    already_happened = True
        return already_happened

    def get_next_available_player(
        self, player_a, next_player_index, non_available_players
    ):
        ...
        while (
            self.check_if_match_already_happened(
                player_a, self.tournament.players[index]
            )
            or self.tournament.players[index] in non_available_players
        ):
            index += 1
